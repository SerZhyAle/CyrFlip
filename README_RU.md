...
CyrFlip - крошечная утилита для Windows (живёт в трее) со двумя скромными задачами:

1. **Индикатор раскладки там, где вы печатаете (главная функция).** Маленький маркер **EN / RU / UK** показывается и на текстовом курсоре мыши (I-beam), **и рядом с мигающей кареткой** - чтобы вы всегда видели, в какой раскладке набираете. Идея в том, чтобы взглянуть на него *до* того, как целый абзац уйдёт не в тот алфавит. Обновляется в реальном времени. (Указатель мыши во время набора часто остаётся стрелкой - поэтому маркер у каретки особенно важен.)
2. **Транслитерация одной клавишей.** На случай, если вы всё-таки проигнорировали функцию №1: текст, набранный в неправильной раскладке, можно «перевернуть» на месте между QWERTY и ЙЦУКЕН (EN ↔ RU, UK в планах) горячей клавишей - без перепечатывания и без стыда.
...
3. **История буфера** включается только из меню трея. Это узкое окно поверх других с Unicode-текстами: запись выбирается мышкой, её можно закрепить или удалить; показать/скрыть окно можно через **Ctrl+Shift+F10**. Локальный журнал зашифрован Windows DPAPI, а захват можно поставить на паузу или полностью очистить.
...
- **Запуск** - запустите `CyrFlip.exe`. Окна нет (любоваться нечем); программа тихо сидит в области уведомлений (системном трее). Значок показывает активную раскладку (**EN/RU/UK**).
- **Исправление текста** - выделите тот самый `ghbdtn`, который задумывался как «привет», нажмите **Ctrl+Shift+F12** - и он заменится на месте. Работает в любом приложении (Блокнот, Word, браузеры, ..).
