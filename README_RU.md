![CyrFlip — исправление текста в неправильной раскладке](assets/banner.png)

# CyrFlip

CyrFlip — крошечная фоновая утилита для Windows, исправляющая текст, набранный в
неправильной раскладке. Выделите испорченный текст, нажмите горячую клавишу — и CyrFlip
транслитерирует его между QWERTY и ЙЦУКЕН (EN ↔ RU, UK в планах) прямо на месте.
Утилита живёт в системном трее, а её значок показывает текущую раскладку (EN/RU/UK).

## Статус

Ранняя разработка. Полная спецификация —
[PLAN/KeyboardTransliterator_Specification_v1.0.md](PLAN/KeyboardTransliterator_Specification_v1.0.md),
архитектура и соглашения — [CLAUDE.md](CLAUDE.md).

## Как это работает

1. Глобальный низкоуровневый хук клавиатуры слушает горячую клавишу (по умолчанию **Ctrl+Shift+T**).
2. По срабатыванию выделение копируется, транслитерируется и вставляется обратно.
3. Значок в трее отслеживает активную раскладку и обновляется в реальном времени.

## Использование

- **Запуск** — запустите `CyrFlip.exe`. Окна нет; программа сидит в области уведомлений (системном трее). Значок показывает активную раскладку (**EN/RU/UK**).
- **Исправление текста** — выделите текст в неправильной раскладке, нажмите **Ctrl+Shift+T** — и он заменится на месте. Работает в любом приложении (Блокнот, Word, браузеры, …).
- **Меню в трее** (правый клик по значку):
  - **Flip EN ⇄ RU: Ctrl+Shift+T** — показывает текущую горячую клавишу.
  - **Start with Windows** — запуск CyrFlip при входе в систему (для текущего пользователя, без прав администратора).
  - **Exit** — выход.

CyrFlip — обычное настольное приложение, а не служба Windows: глобальный хук клавиатуры и индикатор раскладки должны работать в интерактивном сеансе пользователя, поэтому «автозапуск» — это запись в автозагрузке пользователя, а не служба.

## Требования

- Windows 10 / 11 (x64)
- .NET Framework 4.8 (предустановлен в Windows 10/11 — ничего доустанавливать не нужно)

## Сборка и запуск

```powershell
dotnet build CyrFlip.sln -c Release
.\src\CyrFlip\bin\Release\net48\CyrFlip.exe
```

Запуск тестов:

```powershell
dotnet test CyrFlip.sln
```

## Конфигурация

Необязательный `config.json` (рядом с exe или в `%APPDATA%\CyrFlip\`):

```json
{ "hotkey": "Ctrl+Shift+T", "layouts": ["EN", "RU"], "cursorSize": 24 }
```

## Лицензия

MIT — см. [LICENSE](LICENSE).
