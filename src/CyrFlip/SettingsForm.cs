...
namespace CyrFlip
{
    ...
    internal sealed class SettingsForm : Form
    {
        ...
        private static void AddDetailedTranslations(Dictionary<string, string> en, Dictionary<string, string> ua)
        {
            ...
            en["Запоминает, открыто ли окно менеджера буфера: если вы его закрыли, при следующем запуске оно останется закрытым (история всё равно ведётся в фоне)."] = "Remembers whether the clipboard manager window is open: if you closed it, it stays closed on the next launch (history is still captured in the background).";
            ua["Запоминает, открыто ли окно менеджера буфера: если вы его закрыли, при следующем запуске оно останется закрытым (история всё равно ведётся в фоне)."] = "Запам'ятовує, чи відкрите вікно менеджера буфера: якщо ви його закрили, під час наступного запуску воно лишиться закритим (історія все одно ведеться у фоні).";
