...
- `src/CyrFlip/` - the app (net48 WinForms): `TransliterationEngine`, `CaseFlipEngine`, `Hotkey`, `KeyboardHook`, `ClipboardHandler`, `CursorIndicator` (layout indicator), `Autostart`, `AppConfig`, `WindowInterop`, the `CyrFlipContext` tray shell, and the `Launcher*` module family (the absorbed OneClickRunner scenario launcher, feature 5 below) - all implemented.
