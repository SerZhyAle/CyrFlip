...
- `docs/` - GitHub Pages site (served from `/docs`). `index.html` is the trilingual EN/RU/UA entry (a CSS `data-lang` switch); the **full guide stays EN/RU/UK** (`guide.html`, `docs/ru/`, `docs/uk/`), while each of the other 10 UI languages gets a **standalone first page** at `docs/<lang>/index.html` with its own `lang`/`dir`, hreflang alternates and a 13-language switcher. Those pages are generated copy - edit them directly, there is no build step. `winget/` - manifest templates. `.github/` - CI + release workflows, issue/PR templates, dependabot. `build.ps1` - local build + deploy.
- `PLAN/` - the spec.
...
5. **Scenario launcher ("Быстрый запуск") - the absorbed OneClickRunner, opt-in and off by default.**
   ...
   **named pipe** (`LauncherIpc`, `CyrFlip_Launcher_Pipe`) and exits - or, with no live instance, does a
   ...
   [PLAN/OneClickRunner_Absorption_Spec_Idea_v0.1.md](PLAN/OneClickRunner_Absorption_Spec_Idea_v0.1.md)
...
6. **Built-in translator on a local Ollama ("Перевод"), opt-in and off by default.** `EnableTranslate=0`
   ...
   **An MSIX build never downloads or runs the Ollama installer** (`OllamaManager.CanInstallInPlace` -
   ...
   [PLAN/Translator_Spec_Idea_v0.1.md](PLAN/Translator_Spec_Idea_v0.1.md).
...
7. **CyrFlip's own context menu over the selection ("Контекстное меню"), opt-in and off by default.**
   ...
   whole feature exists to keep. Spec: [PLAN/ContextMenu_Spec_Idea_v0.1.md](PLAN/ContextMenu_Spec_Idea_v0.1.md).
...
- **TranslationProfile.cs** - one table row (`TargetLang`, `Hotkey`, `Enabled`, an `Id` that survives edits)
  ...
  never Windows' literal "Unknown Language") and `Label` (the endonym the UI shows).
...
| `TranslateModel` | `REG_SZ` | `qwen2.5:3b` | Model name; empty = use whichever model is installed |
