...
- `docs/` - GitHub Pages site (served from `/docs`). `index.html` is the trilingual EN/RU/UA entry (a CSS `data-lang` switch); the **full guide stays EN/RU/UK** (`guide.html`, `docs/ru/`, `docs/uk/`), while each of the other 10 UI languages gets a **standalone first page** at `docs/<lang>/index.html` with its own `lang`/`dir`, hreflang alternates and a 13-language switcher. The **privacy policy exists in all 13** (`docs/<lang>/privacy.html`; `privacy.html` at
  ...
  at it in every language. All twelve translations are structurally identical (11 list items, 7 sections), which is the
...
**repeated** rebuilds, since the first can never fail).
...
- **Desktop-driving checks** (`tools/uitest/`, PowerShell): the interop that xUnit cannot reach -
  ...
  opens Settings) and `Save-SettingsShots.ps1` (a PNG per settings tab, for layout/localization
  eyeballing). Deliberately outside `dotnet test`/CI: they move the real mouse and steal focus.
