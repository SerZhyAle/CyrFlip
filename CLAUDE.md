...
- **ClipboardHistoryService.cs (+ `ClipboardHistoryWindow` / `ClipboardHistorySearchWindow`)** - the opt-in
  ...
  **Accepted design decision (2026-07-26, after a leak audit): the history is deliberately unbounded.**
  ...
  oblige is that nothing may cost O(history) per copy or per repaint: repeat copies are found through the
  `_byUuid` index, the display order is cached in `_ordered` and invalidated by the single `RaiseChanged`
  helper, and each change raises `Changed` exactly once (`SetCurrent` deliberately raises nothing).
...
- **Launcher (absorbed OneClickRunner), one concern per file:**
  ...
  - **LauncherLog.cs** - `launcher.log` in the same MSIX-aware folder as `layout.txt`; never logs the yt-dlp link.
...
subject survives, and that a non-ASCII path never reaches the ANSI API as-is).
...
- **Desktop-driving checks** (`tools/uitest/`, PowerShell): the interop that xUnit cannot reach -
  ...
  one thing only a human can confirm) and `Save-SettingsShots.ps1` (a PNG per settings tab, for
  layout/localization eyeballing). Deliberately outside `dotnet test`/CI: they move the real mouse and steal focus.
  `CyrFlip.UiTest.psm1` holds the reusable seams (PerMonitorV2 DPI, MSAA tray-icon lookup,
  synthesized clicks, per-window HKL, `PrintWindow` screenshots) - **write new manual checks on top
