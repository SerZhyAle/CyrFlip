...
- **KeyboardHook.cs** - `SetWindowsHookEx(WH_KEYBOARD_LL)` wrapper. The callback ignores injected events (`LLKHF_INJECTED`) so our own `SendInput` can't re-enter it, matches **two** chords via `GetAsyncKeyState` (`Matches(hotkey, vk)`), raises `HotkeyPressed` (flip) or `CaseHotkeyPressed` (case flip), and returns `1` to **swallow** the trigger key. `Install(flip, caseFlip)`; `UpdateHotkey`/`UpdateCaseHotkey` swap a matched chord at runtime (safe from any thread - the callback reads the fields on each invocation).
